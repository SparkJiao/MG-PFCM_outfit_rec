...
class Data(object):
    def __init__(self, path):
        ...
        recom_test_file_neg = path + '/test_uo_neg.txt'
        recom_test_file_neg_mrr = path + '/test_uo_neg_mrr.txt'

        self.n_train_items, self.outfit_items_dict, self.max_ol = self._read_outfit_file(outfit_file)
        #print('====================',max(self.outfit_items_dict.keys()))
        #sys.exit()
        ...
        self.n_train_outfits = max(self.outfit_items_dict.keys()) #======设置为最大的id
        ...
        self.n_recom_tests, self.test_u_outfits_dict,self.n_test_outfits = self._read_recom_test_file(recom_test_file)
        self.test_u_outfits_dict_neg,self.n_test_outfits_neg = self._read_recom_test_file_neg(recom_test_file_neg)
        self.test_u_outfits_dict_neg_mrr,self.n_test_outfits_neg_mrr = self._read_recom_test_file_neg_mrr(recom_test_file_neg_mrr)
    ...
    def get_R_mat(self, uo_dict): 
        #======这里补上+self.n_test_outfits+self.n_test_outfits_neg)  我们有部分outfit是只有测试集里面存在
        #R_uo = sp.dok_matrix((self.n_users, self.n_train_outfits+self.n_test_outfits+self.n_test_outfits_neg), dtype=np.float32)
        #=======这里上面设定了，所以不需要补上额外的了
    ...
    def construct_outfit_map(self,outfit_dict, cate_adj, item_cate):
        ...
        outfit_len = []
        outfit_adj = []
        for o in range(self.n_train_outfits): #========补上
            items = outfit_dict[o]
            o_len = len(items)
    ...
    def _read_recom_test_file(self, test_file):
        n_tests = 0
        test_u_outfits = {}
        #for u in range(self.n_users):  #并不是所有的user都在test中出现过
        #    test_u_outfits[u] = []
        ...
        n_test_outfits = len(test_u_outfits)
        return n_tests, test_u_outfits,n_test_outfits

    def _read_recom_test_file_neg(self, test_file):  #读取预设的负例
        #n_tests = 0
        test_u_outfits = {}
        #for u in range(self.n_users):  #并不是所有的user都在test中出现过
        #    test_u_outfits[u] = []
        ...
        n_test_outfits_neg = len(test_u_outfits)
        return test_u_outfits,n_test_outfits_neg


    def _read_recom_test_file_neg_mrr(self, test_file_mrr):  #读取预设的负例
        #n_tests = 0
        test_u_outfits = {}

        with open(test_file_mrr) as f:
            for l in f.readlines():
                if len(l) > 0:
                    l = l.strip('\n')
                    try:
                    ...
                    uid, test_outfits = outfits[0], outfits[1:]
                    if uid not in test_u_outfits:
                        test_u_outfits[uid] = []
                    test_u_outfits[uid].append(test_outfits)
                    #n_tests += len(test_outfits)
    ...
    def _read_recom_train_file(self, train_file):
        ...
        n_train_outfits = max(train_outfit_set)  #这里改成max 取最大的
