...
_n_batch = None
_n_tests = None
#_cores = multiprocessing.cpu_count() // 2   #先不开多核
_cores = 1
_max_ol = None
...
def get_train_batch(b):
    ...
    for p in range(begin, end):
        ...
        neg_o = pos_o
        while neg_o in _data_generator.train_u_outfits_dict[u] or (u in _data_generator.test_u_outfits_dict and neg_o in _data_generator.test_u_outfits_dict[u]):
        ...
        #print('_max_ol========',_max_ol)

        #neg_len = rd.randint(1, _max_ol)   # 源代码是(3, _max_ol)
        #这里的设置会报错，因为我们的套装内服装数目是2，这里（3,2）会报错，或许可以尝试设置为1
        neg_len = _max_ol
