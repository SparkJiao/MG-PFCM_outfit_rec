...
class GPBPR(Module):
    def __init__(self, user_set, item_set, embedding_weight ,
        max_sentence = 83,  text_feature_dim=768, 
    ...
    def forward(self, batch, visual_features, text_features, **args):
        ...
        if not self.visual_nn[0].weight.data.is_cuda:
            ...
            I_text_latent = self.textcnn( 
                self.text_embedding( 
                    cat(
                        [text_features[I].unsqueeze(0).to(torch.int64) for I in Is], 0
                    )
                ) .unsqueeze_(1)
            )
            J_text_latent = self.textcnn( 
                self.text_embedding( 
                    cat(
                        [text_features[J].unsqueeze(0).to(torch.int64) for J in Js], 0
                    )
                ).unsqueeze_(1)
            )
            K_text_latent = self.textcnn( 
                self.text_embedding( 
                    cat(
                        [text_features[K].unsqueeze(0).to(torch.int64) for K in Ks], 0
                    )
                ) .unsqueeze_(1)
            )
    ...
    def forward_mrr2(self, batch, visual_features, text_features, **args):
        # pre deal
        Us = [str(int(pair[0])) for pair in batch]
        Is = [str(int(pair[1])) for pair in batch]
        Js = [str(int(pair[2])) for pair in batch]
        ...
        p_ik = 0.5 * visual_ik + 0.5 * text_ik
        # union
        score1 = (self.uniform_value * p_ij + (1 - self.uniform_value) * cuj \
            - ( self.uniform_value * p_ik + (1 - self.uniform_value) * cuk )).le(0)

        #print('res0:==',score1.int())
        #res = torch.sum(score1)  #计算正例的index，所有为1的代表分数比正例大
        #print('res1:==',res)
        res = torch.add(score1,1)
        res = 1.0/res
        ...
        return self.uniform_value * p_ij + (1 - self.uniform_value) * cuj \
            - ( self.uniform_value * p_ik + (1 - self.uniform_value) * cuk ) ,\
                cujkweight + self.text_embedding( 
                            cat(
                                [text_features[J].unsqueeze(0).to(torch.int64) for J in set(Is+Js+Ks)], 0
                            ).cuda()
                        ).norm(p=2)
