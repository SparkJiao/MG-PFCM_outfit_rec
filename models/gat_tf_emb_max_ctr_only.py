...
class GATTransformer(Module, LogMixin):
    ...
    def forward(self,
        ...
        image_emb = self.resnet(item_image.contiguous())
        ...
        if not self.training:
            acc, true_label_num = get_accuracy(sim_matrix, ctr_labels)
            self.eval_metrics.update("acc", val=acc, n=true_label_num)
            self.eval_metrics.update("loss", val=ctr_loss.item(), n=true_label_num)
