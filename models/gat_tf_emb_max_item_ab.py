...
class GATTransformer(Module, LogMixin):
    def __init__(self,
                 ...
                 item_use_img: bool = True,
                 item_use_text: bool = True,
                 gnn: Module = None,
        ...
        # Item text embedding
        item_input_size = 0
        self.item_use_img = item_use_img
        self.item_use_text = item_use_text
        if self.item_use_img:
            item_input_size += img_hidden_size
        if self.item_use_text:
            item_input_size += text_hidden_size
        self.item_proj = nn.Linear(item_input_size, hidden_size)
    ...
    def forward(self,
        ...
        item_input = []
        if self.item_use_img:
            item_input.append(image_emb)
        if self.item_use_text:
            item_input.append(item_text)
        item_emb = self.item_proj(torch.cat(item_input, dim=-1))
    ...
    def predict(self,
        ...
        u_h, i_h = g_h[:, 0], g_h[:, 1]
        cat_h = g_h[:, 2:]
        cat_num = cat_h.size(1)

        u_h = u_h.unsqueeze(1).expand(-1, cat_num, -1)
        i_h = i_h.unsqueeze(1).expand(-1, cat_num, -1)
        triplets = torch.cat([u_h, i_h, cat_h], dim=-1)
